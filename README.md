...
### 기능
   ...
   * 로그인
   ...
   예외흐름
   E1) 아이디가 일치하지 않는 경우
    일치하는 아이디가 없다는 경고창을 보여주고 로그인 페이지로 이동한다.

   E2) 비밀번호가 일치하지 않는 경우
    비밀번호가 일치하지 않는다는 없다는 경고창을 보여주고 로그인 페이지로 이동한다.
    ```
...
먼저 초기 페이지인 index 페이지로 들어오도록 만듭니다. 초기 페이지에는 현재 상영 중인 영화를 검색하는 검색배너, 현재 상영중인 영화 중 인기있는 영화 목록을 보여주는 배너, 공지사항 및 이벤트 목록, Contact 정보를 보여줍니다.

index 페이지에서 영화 예매, 검색, 추천과 리뷰 작성 및 조회, 회원 수정을 할 수 있습니다. 이 때, 검색은 로그인이 없어도 가능하지만 그 외 나머지 기능들은 로그인이 되어 있어야만 가능합니다.
...
로그인을 하면 내비바의 아이콘을 통해 내역 조회를 할 수 있다. 내역조회를 누르면 첫 페이지에 최근 3개월 간의 예매 내역과 결제 내역이 회원에게 나타난다. 예매 내역과 결제 내역 각각 1주일, 1개월, 3개월, 6개월, 1년 단위로 내역을 검색할 수 있다.

#### 6. 회원 정보 관리 페이지

로그인을 하면 내비바의 아이콘을 통해 회원 정보 관리페이지로 이동할 수 있다. 회원 페이지의 초기 화면은 현재 설정된 회원 정보 내용들이 회원에게 보여진다. 현재 정보 밑에 있는 입력 창에 수정정보를 입력한 후 확인 버튼을 누른다.

### 각종 기능들 구현 방법

> 기본적으로 현 프로젝트는 Spring framework를 사용하여 구현하고 있습니다.
